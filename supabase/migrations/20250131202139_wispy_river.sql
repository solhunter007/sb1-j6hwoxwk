...
CREATE OR REPLACE FUNCTION get_comment_count_v6(p_sermon_note_id UUID)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION handle_comment_count_change_v6()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
DROP TRIGGER IF EXISTS handle_comment_count_change_v6_trigger ON comments;

-- Create new trigger for comment count updates
CREATE TRIGGER handle_comment_count_change_v6_trigger
  AFTER INSERT OR DELETE ON comments
  FOR EACH ROW
  EXECUTE FUNCTION handle_comment_count_change_v6();