...
CREATE OR REPLACE FUNCTION get_comment_count_v10(p_sermon_note_id UUID)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION handle_comment_count_change_v10()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
DROP TRIGGER IF EXISTS handle_comment_count_change_v10_trigger ON comments;

-- Create new trigger for comment count updates
CREATE TRIGGER handle_comment_count_change_v10_trigger
  AFTER INSERT OR DELETE ON comments
  FOR EACH ROW
  EXECUTE FUNCTION handle_comment_count_change_v10();
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE indexname = 'idx_comments_sermon_note_id_v10'
  ) THEN
    CREATE INDEX idx_comments_sermon_note_id_v10 
