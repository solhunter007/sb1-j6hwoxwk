/*
  # Add Comment Count Logging

  1. Changes
    - Add logging for comment count operations
    - Add validation and repair logging
...
-- Function to get comment count with logging
CREATE OR REPLACE FUNCTION get_comment_count_v14(p_sermon_note_id UUID)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Function to handle comment changes with detailed logging
CREATE OR REPLACE FUNCTION handle_comment_count_change_v14()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
DROP TRIGGER IF EXISTS handle_comment_count_change_v14_trigger ON comments;

-- Create new trigger for comment count updates
CREATE TRIGGER handle_comment_count_change_v14_trigger
  AFTER INSERT OR DELETE ON comments
  FOR EACH ROW
  EXECUTE FUNCTION handle_comment_count_change_v14();